...
export function CharacterProvider({ children }: { children: React.ReactNode }) {
    ...
    const loadCharacterData = async () => {
      ...
      try {
        ...
        if (characterDoc.exists()) {
          ...
          // Process loaded equipped items
          Object.entries(loadedEquippedItems).forEach(([slot, itemUntyped]) => {
            ...
            if (item) { // Check if item data exists
              ...
              if (item.itemType === 'Weapon' && (slotKey === 'primaryWeapon' || slotKey === 'secondaryWeapon')) {
                ...
                const typedWeapon: WeaponItem = {
                  ...
                  // Use the exact values from Firebase for simple fields
                  meleeRanged: item.meleeRanged as 'Melee' | 'Ranged', 
  ...
  // Add default values for damageAmount and damageType
  ...
  return { 
    ...
    name: `${weapon.name} Attack (${slot})`, 
    description: `Attack with your ${weapon.name} in ${slot.toLowerCase()} hand.`, 
    damageAmount: weapon.damageAmount || 'N/A', // Add default value
    damageType: weapon.damageType || 'Physical', // Add default value
    meleeRanged: weapon.meleeRanged,
    ...
    handsRequired: weapon.handsRequired || 'One-handed',
    slot: 'primaryWeapon'
  }; 
}, []);
  ...
  useEffect(() => { setAttacks(getAttacksFromEquipment()); }, [getAttacksFromEquipment]); // Update attacks when equipment changes
