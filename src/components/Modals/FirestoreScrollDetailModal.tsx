// FirestoreScrollDetailModal.tsx
...
const FirestoreScrollDetailModal: React.FC<ScrollDetailModalProps> = ({ scroll, isOpen, onClose }) => {
  const [currentLevel, setCurrentLevel] = useState(1);
  const [completeScrollData, setCompleteScrollData] = useState<any>(null);
  ...
  useEffect(() => {
    ...
    if (isOpen && scroll) {
      ...
      // Fetch complete scroll data from Firestore
      const fetchScrollData = async () => {
        if (!scroll.id) {
          setError("Scroll ID is missing");
          return;
        }
        ...
        try {
          console.log(`Fetching complete data for scroll ID: ${scroll.id}`);
          const scrollRef = doc(db, 'scrolls', scroll.id);
          const scrollDoc = await getDoc(scrollRef);
          
          if (scrollDoc.exists()) {
            const fullData = {
              ...scrollDoc.data(),
              ...scroll // Preserve original data in case Firestore is missing something
            };
            console.log("Complete scroll data fetched:", fullData);
            setCompleteScrollData(fullData);
          } else {
            console.warn(`Scroll document with ID ${scroll.id} not found in Firestore`);
            setCompleteScrollData(scroll); // Fallback to original data
          }
        } catch (err) {
          console.error("Error fetching scroll data:", err);
          setError("Failed to load complete scroll data");
          setCompleteScrollData(scroll); // Fallback to original data
        } finally {
          setIsLoading(false);
        }
  ...
  // Use either the complete data from Firestore or the original data
  const scrollData = completeScrollData || scroll;

  if (!scrollData) return null;

  // --- Extract scroll properties with fallbacks for different naming conventions ---
  const scrollName = getScrollProperty(scrollData, ['name']) || 'Unknown Scroll';
  const scrollDescription = getScrollProperty(scrollData, ['description']) || '';
  const scrollEffect = getScrollProperty(scrollData, ['effect', 'spellEffect', 'effectDescription']) || '';
  const scrollRarity = getScrollProperty(scrollData, ['rarity']) || 'Common';
  
  // Time-based properties
  const castingTime = getScrollProperty(scrollData, ['castingTime', 'castTime']) || '-';
  const duration = getScrollProperty(scrollData, ['duration', 'spellDuration']) || '-';
  const cooldown = getScrollProperty(scrollData, ['cooldown', 'spellCooldown']) || '-';
  
  // Resource properties
  const manaCost = getScrollProperty(scrollData, ['manaPointCost', 'manaCost', 'mpCost']) || '-';
  const modifier = getScrollProperty(scrollData, ['spellCastingModifier', 'castingModifier', 'modifier']) || '-';
  
  // Combat properties
  const range = getScrollProperty(scrollData, ['range', 'spellRange']) || '-';
  const damageAmount = getScrollProperty(scrollData, ['damageAmount', 'damage']) || '-';
  const damageType = getScrollProperty(scrollData, ['damageType', 'damagetype', 'damage_type']) || '-';
  
  // Value properties
  const buyValue = getScrollProperty(scrollData, ['buyValue', 'buy_value', 'value']) || '-';
  const sellValue = getScrollProperty(scrollData, ['sellValue', 'sell_value']) || '-';
  
  // --- Scaling Data ---
  let scrollScaling = getScrollProperty(scrollData, ['scaling']) || null;
          ...
          {isLoading ? (
            <Center py={10}>
              <VStack spacing={4}>
                ...
                <Text color="gray.400">Loading scroll details...</Text>
              </VStack>
            </Center>
          ) : error ? (
            <Center py={10}>
...
export default FirestoreScrollDetailModal;