...
import { collection, addDoc, getDocs, query, where, getDoc, doc, updateDoc, deleteDoc, writeBatch, Timestamp, orderBy, serverTimestamp } from 'firebase/firestore'; // Added Timestamp, orderBy, serverTimestamp
import { db } from '@/firebase/firebaseConfig';
import { useAuth } from '@/context/AuthContext';
import { ScrollArea } from '@/components/ui/scroll-area';
import debounce from 'lodash/debounce';
import ItemSelect from '../../loot/ItemSelect';
...
const DMQuestManager: React.FC = () => {
    ...
    // Removed questAssignments state as it's not directly managed here
    ...
    const loadQuests = useCallback(async () => {
        ...
        try {
            ...
            const q = query(questsCollection, orderBy('createdAt', 'desc'));
            ...
            const questsList = questsSnapshot.docs.map(docSnap => ({
                ...
                rewards: {
                    ...(docSnap.data().rewards || {}),
                    items: docSnap.data().rewards?.items || [],
                    other: docSnap.data().rewards?.other || '', // Ensure 'other' is loaded
                },
                createdAt: docSnap.data().createdAt || Timestamp.now(), // Use Firestore Timestamp
            } as Quest));
            setQuests(questsList);
        } catch (error) {
            console.error('Error loading quests:', error);
            toast({ title: 'Error Loading Quests', status: 'error' });
        } finally {
    ...
    const loadCharacters = useCallback(async () => {
        ...
        try {
            ...
            const charactersList = charactersSnapshot.docs.map(docSnap => ({
                ...
                userId: docSnap.data().userId, // User ID who owns the character
                displayName: '', // Placeholder, might need to fetch user data if display name is required
                level: docSnap.data().characterLevel || 1,
            } as PlayerCharacter));

             // Optionally fetch player display names if needed (more complex)
             // For simplicity, we'll just use character name for now in the assignment modal
        ...
        <VStack spacing={4} align="stretch">
            ...
            <Box p={3} borderWidth="1px" borderRadius="md" borderColor="gray.600" bg="gray.750">
                ...
                <FormControl mb={4}>
                    ...
                    {/* ItemSelect handles adding/removing/displaying items */}
                    <ItemSelect
                        selectedItems={currentQuest.rewards?.items || []}
                        onItemSelect={(item, quantity) => {
                            const rewardItem: RewardItem = { ...item, quantity };
                            const existingIndex = (currentQuest.rewards?.items || []).findIndex(i => i.id === item.id);
                            let newItems;
                            if (existingIndex > -1) {
                                newItems = [...(currentQuest.rewards?.items || [])];
                                newItems[existingIndex] = { ...newItems[existingIndex], quantity: (newItems[existingIndex].quantity || 0) + quantity };
                            } else {
                                newItems = [...(currentQuest.rewards?.items || []), rewardItem];
                            }
                        ...
                        allowQuantityEditing={true} // Allow editing quantity directly in ItemSelect if supported
                    />
                </FormControl>
                ...
                <Tbody>
                    {filteredQuests.map(quest => (
                        <Tr key={quest.id} _hover={{ bg: 'gray.750' }}>
                            <Td>
                                <Tooltip label={quest.description} placement="top-start" bg="gray.600" color="white">
                                    <Text fontWeight="medium" noOfLines={1} color="gray.100">{quest.title}</Text>
                                </Tooltip>
                            </Td>
                            <Td><Badge colorScheme={getStatusColorScheme(quest.status)}>{quest.status}</Badge></Td>
                            <Td color="gray.400">{quest.requiredLevel || '-'}</Td>
                            <Td color="gray.400">{quest.location || '-'}</Td>
                            <Td>
                                <HStack spacing={1}>
                                        ...
                                        characters.map(char => (
                                            <HStack key={char.id} p={2} bg="gray.700" borderRadius="md" justify="space-between" _hover={{bg: "gray.600"}}>
                                                <Checkbox isChecked={selectedCharactersForAssign.includes(char.id)} onChange={() => handleToggleCharacterSelect(char.id)} colorScheme="brand">
                                                    {char.characterName} <Text as="span" fontSize="xs" color="gray.400">(User ID: ...{char.userId.slice(-6)})</Text> {/* Show truncated User ID */}
                                                </Checkbox>
                                                <Badge>Lvl {char.level}</Badge>
                                            </HStack>
                                        ))}
                                    </VStack>
                                 </ScrollArea>
